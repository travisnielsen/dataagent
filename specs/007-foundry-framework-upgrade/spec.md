...
**Status**: Implemented on feature branch; pending merge (post-implementation correction landed 2026-05-17 — see [research.md R3/R4/R7 post-implementation correction boxes](research.md))

> **Implementation note (post-implementation 2026-05-17)** — The agent-framework 1.4.x line splits the rc4-era `AzureAIClient` into two distinct classes: `FoundryChatClient` (chat-completions only; no portal-record binding) and `FoundryAgent` (hosted-agent connector; sends `agent_reference` on every Responses API call, bound to a hosted **PromptAgent** record). FR-002 / FR-004 were originally written assuming the chat-client surface alone would carry `agent_reference`; in practice, **the orchestrator and the `query-builder-agent` were rebuilt as `FoundryAgent` instances** to actually populate the portal Traces tab, while `parameter-extractor-agent` remains on `Agent(client=FoundryChatClient)` pending creation of a portal record (deferred follow-up). See [research.md R7](research.md) and [tasks.md Phase 7](tasks.md) for the full mechanism and audit trail.
...
- **FR-004**: Every responses call issued by the chat path on behalf of an agent with a hosted PromptAgent record MUST carry an `agent_reference` (name + version) identifying that record, so the portal Traces tab can correlate the resulting trace to that agent. *(Implementation: this is achieved by constructing the agent as `FoundryAgent(agent_name=...)` rather than `Agent(client=FoundryChatClient)`; the SDK adds `agent_reference` to the Responses API request body automatically.)*
- **FR-005**: The orchestrator agent's name MUST be resolvable from configuration (`AZURE_AI_ORCHESTRATOR_AGENT_NAME`, default `"DataAssistant"`). When the configured record is missing from the Foundry project, the SDK MUST surface a clear `404` on the first turn (returned to the client through the standard SSE error path) rather than silently proceeding without an `agent_reference`. Programmatic upsert of the PromptAgent record is **out of scope for this feature** — records are provisioned manually via the Foundry portal today; an automated upsert helper is tracked as a follow-up under [tasks.md Phase 7](tasks.md).
...
- **Foundry PromptAgent record**: A hosted agent record in the project (created via the Foundry portal) with `kind: "prompt"`. Bound by name from runtime code via `FoundryAgent(agent_name=...)`. The SDK sends `agent_reference` on every Responses API call, which is what the portal Traces tab filters by. This feature relies on two such records: `DataAssistant` (orchestrator) and `query-builder-agent` (workflow). A third record for `parameter-extractor-agent` is **not yet provisioned** — see follow-up in [tasks.md Phase 7](tasks.md).
- **FoundryAgent vs Agent (type widening)**: In agent-framework 1.4.x, `FoundryAgent` (hosted-agent connector) and `Agent` (chat-completions wrapper) are sibling subclasses of `BaseAgent` with no shared public `.run()`-bearing ancestor. Code paths accepting either kind use the `Agent | FoundryAgent` union (`DataAssistant.__init__`, `PipelineClients.query_builder_agent`, `build_query`, `_build_agent_session`). Documented for reviewers as a deliberate widening, not accidental coupling.
...
- **SC-006**: Every chat turn after this feature lands produces a trace visible to feature 008's trace-based evaluation, filterable in the Foundry portal Traces tab by agent name (`DataAssistant`, `query-builder-agent`) and in Application Insights by the OTel attributes the SDK already emits. This is the gate feature 008 needs to begin.

## Assumptions

- The `agent-framework` 1.4.x release line exposes a documented Foundry hosted-agent connector (`FoundryAgent` in `agent_framework.foundry`) whose surface is compatible with the project's current usage of `AzureAIClient` (instructions + tools + streaming). Verified during implementation; pinned in [plan.md](plan.md) / [research.md R1, R7](research.md).
- The Foundry project that Cadence targets supports the Responses protocol and the `agent_reference` body-field pattern documented at [Microsoft Foundry — Build with agents, conversations, and responses](https://learn.microsoft.com/azure/foundry/agents/concepts/runtime-components) and at [Add client-side tracing to Foundry agents (preview)](https://learn.microsoft.com/azure/foundry/observability/how-to/trace-agent-client-side).
- Hosted PromptAgent records (`DataAssistant`, `query-builder-agent`) are provisioned in the project out-of-band via the Foundry portal. Operators are responsible for keeping the record name in sync with `AZURE_AI_ORCHESTRATOR_AGENT_NAME`. **`FoundryAgent(instructions=...)` overrides the portal-stored prompt per-request**, so the runtime source of truth for agent instructions remains the local `prompt.md` / `assistant_prompt.md` markdown files; portal copies are decorative until a sync helper is built.
- A single orchestrator agent record per deployment is sufficient for portal correlation of the chat path. `query-builder-agent` is registered as its own record because a hosted PromptAgent record already exists; `parameter-extractor-agent` is intentionally **not** promoted (no record exists; spans still flow to Application Insights with the right `gen_ai.*` attributes — just no per-agent Traces view in the portal).
