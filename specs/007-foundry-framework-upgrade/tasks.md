...
## Phase 7 — Post-implementation correction: portal trace correlation

**Discovered after T018 manual smoke (2026-05-17)**: With T017 landed (`Agent(client=FoundryChatClient(...), id=..., name=...)`), the Foundry portal **Traces** tab for `DataAssistant` loaded cleanly but stayed empty across multiple multi-turn smokes. Spans were reaching Application Insights with the right `gen_ai.*` attributes, but the portal tab populates from a different signal: the `agent_reference` field on the **Responses API request body**, which is only emitted by `RawFoundryAgentChatClient` (the chat-client backing `FoundryAgent`), not by `FoundryChatClient`. The 2026-05-19 R3 conclusion in [research.md](research.md) ("the OTel attributes alone are sufficient") was incorrect; see the **R3 / R7 post-implementation correction** boxes there for the full mechanism.

**Resolution**: swap `Agent(client=FoundryChatClient(...))` → `FoundryAgent(project_endpoint=..., agent_name=..., instructions=...)` for the agents that have hosted PromptAgent records in the portal (`DataAssistant`, `query-builder-agent`). Keep `parameter-extractor-agent` on the chat-client form pending creation of a portal record.

- [x] T025 [US2] Replace orchestrator construction in [src/backend/api/routers/chat.py](src/backend/api/routers/chat.py): drop `Agent(client=FoundryChatClient(...))`; build `FoundryAgent(project_client=project_client, agent_name=settings.azure_ai_orchestrator_agent_name, id=settings.azure_ai_orchestrator_agent_id, name=..., instructions=load_assistant_prompt())`. Construct the shared `AIProjectClient` (from `azure.ai.projects.aio`) up-front and pass it both to `FoundryAgent` and to the existing `openai_client.conversations.create()` pre-create block (which still runs unchanged). Remove the `inspect.isawaitable` workaround from the streaming loop \u2014 `FoundryAgent.run(...)` is consistently async.
- [x] T026 [P] [US2] Add `azure_ai_orchestrator_agent_name: str = "DataAssistant"` to `Settings` in [src/backend/config/settings.py](src/backend/config/settings.py). Note: this **replaces** the planned default of `cadence-data-assistant` from T004 \u2014 the portal record actually shipped as `DataAssistant`. `azure_ai_orchestrator_agent_id` remains optional (`str | None = None`); when set, it pins to a specific agent version. Update [src/backend/.env.example](src/backend/.env.example) accordingly.
- [x] T027 [P] [US2] Rewrite [src/backend/query_builder/agent.py](src/backend/query_builder/agent.py): replace `Agent(client=FoundryChatClient(...))` with a `FoundryAgent` factory `create_query_builder_agent(*, project_endpoint, credential, instructions)` and a module-level singleton. Export `QUERY_BUILDER_AGENT_NAME = "query-builder-agent"` for use by callers. The portal record `query-builder-agent` already exists from an earlier rollout.
- [x] T028 [P] [US2] Update [src/backend/workflow/clients.py](src/backend/workflow/clients.py): import `FoundryAgent` alongside `Agent`; widen `PipelineClients.query_builder_agent` type to `Agent | FoundryAgent`; build it via `create_query_builder_agent(...)` instead of `FoundryChatClient` + `Agent(...)`. Leave `param_extractor_agent: Agent` unchanged (still wraps `FoundryChatClient`) and add a comment explaining the deferral.
- [x] T029 [P] [US2] Widen the `agent` parameter type on the call-site signatures that now accept either kind: `DataAssistant.__init__(agent: Agent | FoundryAgent, ...)` ([src/backend/assistant/assistant.py](src/backend/assistant/assistant.py)), `build_query(request, agent: Agent | FoundryAgent, ...)` ([src/backend/query_builder/builder.py](src/backend/query_builder/builder.py)), `_build_agent_session(agent: Agent | FoundryAgent, ...)` ([src/backend/nl2sql_controller/pipeline.py](src/backend/nl2sql_controller/pipeline.py)). Import `FoundryAgent` in each file. *Rationale*: `FoundryAgent` and `Agent` are sibling subclasses of `BaseAgent` with no shared public `.run()`-bearing ancestor; the union is required by basedpyright in strict mode (see [research.md R7](research.md)).
- [x] T030 [US2] Update test patches in [tests/unit/test_sse_endpoint.py](tests/unit/test_sse_endpoint.py): `_ORCH_PATCHES` drops the `"Agent"` and `"AzureAIClient"` keys; adds `"FoundryAgent": "agent_framework.foundry.FoundryAgent"` and `"AIProjectClient": "azure.ai.projects.aio.AIProjectClient"`. Refactor `test_creates_new_assistant_when_cache_miss` and `test_uses_managed_identity_when_client_id_set` to assert against `mock_foundry_agent_cls.assert_called_once()` and `mock_project_client_cls.assert_called_once()`. All 620 unit tests pass.
- [x] T031 [US2] Manual portal verification (re-run of T018): drive ≥ 2 turns through the dev API, open Foundry portal → Agents → `DataAssistant` → Traces. **Verified**: row appears within ~1 minute per turn; per-turn distinct trace IDs share the same `conv_…` conversation_id; agent column populated. Same verification for `query-builder-agent` Traces tab on a turn that exercises dynamic SQL generation. SC-003 + SC-006 satisfied.

### Deferred follow-up (not gating this feature)

- [ ] T032 [follow-up] Provision a PromptAgent record for `parameter-extractor-agent` (manual via Foundry portal, or via the MCP `agent_update` tool when network access permits), then promote [src/backend/parameter_extractor/agent.py](src/backend/parameter_extractor/agent.py) from `Agent(client=FoundryChatClient(...))` to `FoundryAgent(agent_name=\"parameter-extractor-agent\", ...)`. Until then, parameter-extractor spans show in Application Insights but not in the per-agent portal Traces view.
- [ ] T033 [follow-up] Build a programmatic upsert helper for PromptAgent records (the original R4 idea, now using the Foundry REST API via `AIProjectClient.send_request` against `/agents` with `kind: \"prompt\"`; the `agent_definition_schema_get` MCP tool returns the JSON schema). Would replace the manual portal-create step and let CI bring up disposable preview environments.

**Checkpoint**: Portal Traces tab populates for `DataAssistant` and `query-builder-agent`. Feature 008 is unblocked. Spec, plan, research, data-model, quickstart, and tasks are reconciled with what shipped on the feature branch (commit `166c4e7` and follow-ups).
